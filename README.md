...
- **Optional web search with citations** (research-focused multi-source search; arXiv-only via `--arxiv-only-search`)
- **Speaker notes and arXiv figure insertion**
- **Graphviz flowchart & diagram generation** with intent-aware styles
- **Post-run revision tools** (attach figures/diagrams or edit existing outputs)

### Reliability & Workflow
- Robust slide generation with **automatic retries**
- **Interactive checkpoints** for user feedback (including source approval and slide-title approval)
...
- Topic based research using research-focused multi-source search (arXiv, Semantic Scholar, Crossref, OpenReview) with citations.
- Source approval step (add/remove sources) before outline generation.
- Web-source approval after query-driven search (add/remove sources).
- Slide-title approval with optional slide-count adjustment and extra headings.
- Post-run editing: `--edit-run` to revise existing slides/notes without recompute.
...
### Post-Run Revision
- `--revise-with-figures /path/to/run` re-attaches figures from LaTeX sources based on captions + slide titles.
...
- Expand the topic into a detailed, critical-analysis research query
...
- Use `--topic-scholarly-only` to reduce noise and keep sources to reputable venues (for example - CVPR, ICML, NeurIPS, arXiv, OpenReview, ACL).
...
- `-maxq`, `--max-web-queries` max LLM-generated web search queries per run (default `4`)
- `--arxiv-only-search` use arXiv-only search instead of the multi-source research search
...
- `-msc`, `--max-summary-chunks` cap for LLM summary chunks (default `30`), with semantic chunk ranking to prioritize relevant chunks
...
- Resume a stopped run with `--resume /path/to/run` (or `.../outputs`). Read mode can also resume from the latest `outline-*.json`.
